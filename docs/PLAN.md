...
#### 코인
- [x] 온체인 데이터 수집 — 고래 추적 (Etherscan V2)
- [x] 펀딩레이트 / OI (ccxt — Binance)
- [x] 공포/탐욕 지수 — alternative.me
- [x] 실시간 가격 + 거래량 수집 (ccxt)

#### 미국주식
- [x] SEC EDGAR 내부자 매매 (Form 4)
- [x] 실시간 가격 수집 (Finnhub API)
- [ ] ~~CNN Fear & Greed Index~~ — 서버 IP 차단됨, 비활성화

#### 한국주식
- [x] 외국인/기관 순매수 (pykrx)
- [x] 공매도 잔고 (pykrx)
- [x] DART 내부자 매매 (OpenDartReader)
- [x] 프로그램 매매 동향 (pykrx)
- [x] 네이버 종토방 버즈 (스크래핑)
- [x] 실시간 가격 수집 (네이버 금융 API)

#### 공통
- [x] 매크로 지표 — FRED API (CPI, 금리, VIX, 금리차, 실업률)
- [x] 매크로 지표 — 한국은행 ECOS API (기준금리, CPI, 실업률)
- [x] 소셜 버즈 모니터링 (Reddit RSS)
- [x] 이상 탐지 알고리즘 (z-score + 실시간 가격 감지)
- [x] 시그널 페이지 실데이터 + AI 해석 (뉴스 연동)
- [x] 텔레그램 시그널 알림 (타입별 쿨다운 + 방향전환 즉시)
...
### Phase 6: 백테스팅 — 시그널 검증 시스템

**목표:** "이 시그널이 왔을 때 실제로 돈이 됐냐?"를 숫자로 검증

#### 6-1. 데이터 준비
- [ ] 시그널 발생 시점 기록 (signal 테이블에 이미 쌓이는 중)
- [ ] 시그널 발생 후 가격 변동 추적 (1시간/1일/3일/7일 후 수익률 기록)
- [ ] 크론 추가: `scripts/track_signal_outcome.py` — 과거 시그널의 결과를 역추적하여 저장

#### 6-2. 시그널 성과 분석
- [ ] 시그널 타입별 승률 계산 (bullish 시그널 후 실제 상승 비율)
- [ ] 시그널 타입별 평균 수익률 (1일/3일/7일 후)
- [ ] 마켓별(한국/미국/코인) 성과 비교
- [ ] 시그널 조합 분석 (예: 외국인 순매수 + 기관 순매수 동시 발생 → 승률?)

#### 6-3. 대시보드
- [ ] 시그널 성과표 페이지 (타입별 승률/수익률 테이블)
- [ ] 시그널 신뢰도 자동 조정 (승률 50% 이하 시그널 비활성화/알림 끄기)

#### 6-4. 고도화 (데이터 충분히 쌓인 후)
- [ ] LightGBM 예측 모델 (시그널 조합 → 방향 예측)
- [ ] 가상매매 자동 전략 (백테스트 결과 기반 자동 매수/매도 추천)
- [ ] vectorbt로 전략별 백테스트 시뮬레이션
